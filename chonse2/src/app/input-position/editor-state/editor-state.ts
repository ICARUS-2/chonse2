import AlgebraicNotationMaker from "../../../libs/chess-game-lib/types/algebraic-notation-builder";
...
import { GameState, GameOverReason, GameScore } from "../../../libs/chess-game-lib/types/game-state";
...
import PieceMaterial from "../../../libs/chess-game-lib/types/piece-material";
...
export default class EditorState
{
  ...
  public checkIsGameOver()
  {
    ...
    for( let posKey of this._previousPositionMap.keys() )
    {
      ...
      if (val)
      {
        if (val >= EditorState.DRAW_BY_REPETITION_THRESHOLD)
        {
          this._gameState.isGameOver = true;
  ...
  private _getPositionKey()
  {
    ...
    for(let i = 0; i <= EditorState._FEN_SPLIT_POSKEY_INDEX; i++)
    {
      posKey += fenSplit[i];
      posKey += " ";
    }
  ...
  private _getEnPassantSquareIfExistsFromTurn(fromSquare: string, toSquare: string, turn: boolean) : string
  {
    ...
    const val = turn ? EditorState._WHITE_EP_TRIGGERS.get(key) : EditorState._BLACK_EP_TRIGGERS.get(key);
