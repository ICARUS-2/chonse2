...
export class DatabaseService 
{
  public saveToDatabase(game: BoardState, comment: string = ""): boolean
  {
    ...
    {
      record.result = `${finalGameState.gameState.reason} ${finalGameState.gameState.gameScore}`
    }

    //if the game was evaluated, set its basic estimation/accuracies.
    if (gameEval != undefined)
    {
      record.accuracy = `${gameEval.estimatedElo?.white} / ${gameEval.accuracy.white}% | ${gameEval.estimatedElo?.black} / ${gameEval.accuracy.black}%`
    }
